...
export const CategoryList = ({
  ...
  return (
    <div className={cn("w-full bg-transparent text-zinc-900 p-8", className)}>
        ...
        <div className="space-y-3">
          {categories.map((category) => (
            ...
            >
              <div
                className={cn(
                  "relative overflow-hidden rounded-2xl border bg-zinc-100/60 backdrop-blur-sm transition-all duration-300 ease-in-out cursor-pointer",
                  // Hover state styles
                  hoveredItem === category.id
                    ? 'min-h-[9rem] border-pink-500/40 shadow-lg shadow-pink-500/10 bg-pink-500/5'
                    : 'min-h-[6.5rem] border-zinc-200/60 hover:border-pink-500/30'
                )}
              >
                {/* Corner brackets that appear on hover */}
                {hoveredItem === category.id && (
                  <>
                    <div className="absolute top-3 left-3 w-5 h-5 border-t-2 border-l-2 border-pink-500 rounded-tl-md" />
                    <div className="absolute bottom-3 right-3 w-5 h-5 border-b-2 border-r-2 border-pink-500 rounded-br-md" />
                  </>
                )}
                  ...
                  <div className="flex-1">
                    <h3
                      className={cn(
                        ...
                        hoveredItem === category.id ? 'text-pink-500' : 'text-zinc-900'
                      )}
                    >
                      {category.title}
                    </h3>
                      ...
                      <p
                        className={cn(
                          "mt-1 transition-colors duration-300 text-sm md:text-base",
                           hoveredItem === category.id ? 'text-zinc-700' : 'text-zinc-500'
                        )}
                      >
                        {category.subtitle}
                      </p>
